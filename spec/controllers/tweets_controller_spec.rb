...
describe TweetsController do
  ...
  describe "GET index" do
    it "assigns all tweets as @tweets" do
      Tweet.stub!(:find).with(:all).and_return([mock_tweet])
      get :index
      assigns[:tweets].should == [mock_tweet]
    end
  end

  describe "GET show" do
    it "assigns the requested tweet as @tweet" do
      Tweet.stub!(:find).with("37").and_return(mock_tweet)
      get :show, :id => "37"
      assigns[:tweet].should equal(mock_tweet)
    end
  end

  describe "GET new" do
    it "assigns a new tweet as @tweet" do
      Tweet.stub!(:new).and_return(mock_tweet)
      get :new
      assigns[:tweet].should equal(mock_tweet)
    end
  end
  ...
  describe "POST create" do
    ...
    describe "with valid params" do
      it "assigns a newly created tweet as @tweet" do
        Tweet.stub!(:new).with({'these' => 'params'}).and_return(mock_tweet(:save => true))
        post :create, :tweet => {:these => 'params'}
        assigns[:tweet].should equal(mock_tweet)
      end

      it "redirects to the created tweet" do
        Tweet.stub!(:new).and_return(mock_tweet(:save => true))
        post :create, :tweet => {}
    ...
    describe "with invalid params" do
      it "assigns a newly created but unsaved tweet as @tweet" do
        Tweet.stub!(:new).with({'these' => 'params'}).and_return(mock_tweet(:save => false))
        post :create, :tweet => {:these => 'params'}
        assigns[:tweet].should equal(mock_tweet)
      end

      it "re-renders the 'new' template" do
        Tweet.stub!(:new).and_return(mock_tweet(:save => false))
        post :create, :tweet => {}
  ...
  describe "PUT update" do
    ...
    describe "with valid params" do
      it "updates the requested tweet" do
        Tweet.should_receive(:find).with("37").and_return(mock_tweet)
        mock_tweet.should_receive(:update_attributes).with({'these' => 'params'})
        put :update, :id => "37", :tweet => {:these => 'params'}
      end

      it "assigns the requested tweet as @tweet" do
        Tweet.stub!(:find).and_return(mock_tweet(:update_attributes => true))
        put :update, :id => "1"
        assigns[:tweet].should equal(mock_tweet)
      end
    ...
    describe "with invalid params" do
      ...
      it "assigns the tweet as @tweet" do
        Tweet.stub!(:find).and_return(mock_tweet(:update_attributes => false))
        put :update, :id => "1"
        assigns[:tweet].should equal(mock_tweet)
      end
  ...
  describe "DELETE destroy" do
    ...
    it "redirects to the tweets list" do
      Tweet.stub!(:find).and_return(mock_tweet(:destroy => true))
      delete :destroy, :id => "1"
